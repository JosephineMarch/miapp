    ...
    tasks.forEach(task => {
        ...
        taskElement.innerHTML = `
            <div class="flex items-start">
                <button class="w-6 h-6 rounded-full border-2 ${task.completed ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-indigo-500'} flex items-center justify-center mr-3 mt-1" onclick="toggleTask('${task.id}', ${task.completed})">
...
document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        document.querySelectorAll('.tab-btn').forEach(tab => tab.classList.remove('tab-active', 'text-gray-600'));
        this.classList.add('tab-active');
        
        document.querySelectorAll('#main-content > div[id$="-view"]').forEach(view => view.classList.add('hidden'));
...
document.querySelectorAll('.add-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        let currentView = [...document.querySelectorAll('#main-content > div[id$="-view"]')]
            .find(view => !view.classList.contains('hidden')).id;
