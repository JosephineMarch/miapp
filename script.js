    ...
    collections.forEach(name => {
        ...
        onSnapshot(q, (snapshot) => {
            state[name] = snapshot.docs.map(doc => {
                const data = doc.data();
                // Helper para convertir Timestamps de Firebase a objetos Date de JS de forma segura
...
function handleAddTask() {
    openModal({
        ...
        onSave: (data) => addDoc(collection(db, 'users', currentUser.uid, 'tasks'), {
            ...data,
            completed: false,
...
function handleEditTask(task) {
    openModal({
        ...
        fields: [{ id: 'title', label: texts.task_title, type: 'text', required: true }],
        data: task,
        onSave: (data) => updateDoc(doc(db, 'users', currentUser.uid, 'tasks', task.id), data),
        onDelete: () => deleteTask(task.id)
    });
}

function handleToggleTask(task) {
    updateDoc(doc(db, 'users', currentUser.uid, 'tasks', task.id), { completed: !task.completed });
}
...
function handleAddProject() {
    openModal({
        ...
        fields: [
            ...
            { id: 'description', label: texts.description, type: 'textarea' },
            { id: 'dueDate', label: texts.due_date, type: 'date' },
            { id: 'steps', label: texts.steps, type: 'steps' }
        ],
        onSave: (data) => addDoc(collection(db, 'users', currentUser.uid, 'projects'), {
            ...data,
            createdAt: serverTimestamp()
        })
    });
...
function handleEditProject(project) {
    openModal({
        ...
        onSave: (data) => updateDoc(doc(db, 'users', currentUser.uid, 'projects', project.id), data),
        onDelete: () => deleteDoc(doc(db, 'users', currentUser.uid, 'projects', project.id))
    });
}
...
function handleAddTransaction() {
    openModal({
        ...
        fields: [
            { id: 'description', label: texts.description, type: 'text', required: true },
            { id: 'amount', label: texts.amount, type: 'number', required: true },
            { id: 'type', label: texts.type, type: 'select', options: [{value: 'expense', label: texts.expense}, {value: 'income', label: texts.income}]},
            { id: 'category', label: texts.category, type: 'text' },
            { id: 'date', label: texts.date, type: 'date' }
        ],
        onSave: (data) => addDoc(collection(db, 'users', currentUser.uid, 'transactions'), {
            ...data,
            createdAt: serverTimestamp()
        })
    });
...
function handleEditTransaction(transaction) {
    openModal({
        ...
        onSave: (data) => updateDoc(doc(db, 'users', currentUser.uid, 'transactions', transaction.id), data),
        onDelete: () => deleteDoc(doc(db, 'users',currentUser.uid, 'transactions', transaction.id))
    });
}
    ...
    container.innerHTML = `
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center mb-3">
                ...
                <a href="#" class="text-sm text-indigo-500 hover:underline" onclick="document.querySelector('[data-view=\\'tasks-view\\']').click()">${texts.seeAll}</a>
            </div>
            <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-4">
                 <div class="bg-white rounded-xl p-4 shadow-sm flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">${texts.balance}: $${(state.transactions.reduce((acc, t) => acc + (t.type === 'income' ? t.amount : -t.amount), 0)).toFixed(2)}</p>
                    </div>
                </div>
                <div class="bg-white rounded-xl p-4 shadow-sm flex items-center">
...
function renderProjects() {
    ...
    container.innerHTML = `
        ...
        <div class="space-y-4">
            ${state.projects.length > 0 ? state.projects.map(project => {
                ...
                return `
                <div class="bg-white rounded-xl p-4 shadow-sm card-hover cursor-pointer project-card" data-id="${project.id}">
                    ...
                    ${project.dueDate ? `<div class="text-xs text-gray-500 mb-3"><i class="fas fa-calendar-alt mr-1"></i> ${new Date(project.dueDate).toLocaleDateString()}</div>` : ''}
    ...
    container.querySelectorAll('.project-card').forEach(card => card.onclick = () => handleEditProject(state.projects.find(p => p.id === card.dataset.id)));
}
